...
# Input file for the reference case of pure-shear failure, non-dim values
# eta = 1e23, eta_vk = 1e20
fname = 'out_hyper_notch'

print('# --------------------------------------- #')
print('# Idealised pure-shear model')
print('# --------------------------------------- #')


es = 1e5  # eta scaling used to scale other parameters, a ratio of eta with respect to 10^18
...
dtck = 2.5e-4  # checkpoint of time
maxckpt = 300  # number of checkpoints
tmax = dtck * maxckpt
...
model = ' -L '+str(L)+' -H '+str(H) + ' -xmin '+str(xmin)+\
    ...
    ' -tf_tol ' + str(tf_tol) +\
    ' -poroevo ' + str(poroevo) + ' -gaussian ' + str(gaussian)
